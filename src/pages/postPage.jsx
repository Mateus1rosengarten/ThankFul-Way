...
function PostPage() {
  ...
  const readableDate = format(newDate, "dd/MM/yyyy");
  const isAnyPostTooLong = Object.keys(postIsTooLong).some(
    (key) => postIsTooLong[key]
  );
  let dataWithPosts;
  ...
  const getTweetsFromFirebaseDb = async () => {
    try {
      let searchFromDB = query(
        dataBasecollectionRef,
        where("date", "==", readableDate),
  ...
  const fetchMoreData = async () => {
    ...
    try {
      ...
      const newLastVisible =
        dataWithMorePosts.docs[dataWithMorePosts.docs.length - 1];
  ...
  const sendTweetsToFirebase = async () => {
    ...
    const userPost = await addDoc(dataBasecollectionRef, {
      userName: profileNameValue ? profileNameValue : "Announymus User",
  ...
  return (
    <>
      <div className="div-postPage">
        <span className="post-textArea-mobile">
          For What are You Grateful Today?
        </span>
        {tweetsFromFirebase ? (
          <div className="post-box">
            <div className="post-textArea" type="text">
              <span className="post-textArea">
                For What are You Grateful Today?
              </span>
            </div>
            <div className="div-three-posts">
              <div className="first-post">
                <label className="label-firstPost" htmlFor="firstPost">
                  1:
                </label>
                <input
                  onChange={(event) => {
                    ...
                    setTweetPost({
                      ...
                      postText: {
                        ...tweetPost.postText,
                        firstPost: event.target.value,
                      },
                    });
                  }}
                  ref={inputPostValueRefFirst}
                  className="input-firstPost"
                  id="firstPost"
                />
              </div>
              <div className="second-post">
                <label className="label-secondPost" htmlFor="secondPost">
                  2:
                </label>
                <input
                  onChange={(event) => {
                    ...
                    setTweetPost({
                      ...
                      postText: {
                        ...tweetPost.postText,
                        secondPost: event.target.value,
                      },
                    });
                  }}
                  ref={inputPostValueRefSecond}
                  className="input-secondPost"
                  id="secondPost"
                />
              </div>
              <div className="third-post">
                <label className="label-thirdPost" htmlFor="thirdPost">
                  3:
                </label>
                <input
                  onChange={(event) => {
                    ...
                    setTweetPost({
                      ...
                      postText: {
                        ...tweetPost.postText,
                        thirdPost: event.target.value,
                      },
                    });
                  }}
                  ref={inputPostValueRefThird}
                  className="input-thirdPost"
                  id="thirdPost"
                />
              </div>
            </div>
        ...
        {isAnyPostTooLong && !userHasPosted && !isPostIncomplete && (
          <RedWarningAlert
            textProps={
        ...
        {!isAnyPostTooLong && isPostIncomplete && !userHasPosted && (
          <RedWarningAlert
            textProps={
        ...
        {isAnyPostTooLong ||
          isPostIncomplete ||
          (tweetsFromFirebase?.length === 0 && triggerToShowWarning && (
            <RedWarningAlert
              textProps={
                "It seems there are no posts yet today. Be the first one to share what you're grateful for! 😊"
              }
              divStyleProps={"alert-div-post-warning"}
              spanStyleProps={"alert-text-post-warning"}
            />
